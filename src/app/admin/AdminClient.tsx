...
// Admin nav — grouped to reduce cognitive load. "Primary" sections
// users actually hit daily; "Advanced" gated behind a toggle so the
// sidebar doesn't become a dump. Every id must still exist in the
// NavSection union so the section switch in the body stays compilable
// — we didn't remove panels, just relegated rarely-used ones.
const NAV_PRIMARY: { id: NavSection; icon: React.ElementType; label: string }[] = [
  { id: "overview",   icon: TrendingUp,    label: "Prehľad" },
...
const NAV_ADVANCED: { id: NavSection; icon: React.ElementType; label: string }[] = [
...
const NAV = [...NAV_PRIMARY, ...NAV_ADVANCED];
  ...
  return (
    <div className="flex h-screen overflow-hidden relative" style={{ background: A.bg, color: "#f1f5f9" }}>
      ...
      <aside className="w-16 md:w-64 flex-shrink-0 flex flex-col h-full z-10"
        ...
        {/* Nav — primary group, then optional "Advanced" disclosure */}
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          {NAV_PRIMARY.map(({ id, icon: Icon, label }) => {
          ...
          {/* Quick external links that used to live in the footer */}
          <Link
            href="/admin/feedback"
            className="w-full flex items-center gap-3 px-3 py-3 rounded-2xl transition mt-1"
            style={{ color: P.muted }}
          >
          ...
          <details className="mt-3 group">
            <summary
              className="cursor-pointer flex items-center gap-2 px-3 py-2 rounded-lg text-[0.65rem] uppercase tracking-widest select-none"
              style={{ color: P.faint }}
            >
            ...
            <div className="mt-1 space-y-1">
              {NAV_ADVANCED.map(({ id, icon: Icon, label }) => {
                const active = section === id;
                return (
                  <button key={id} onClick={() => setSection(id)}
                    ...
                    <Icon className="w-3.5 h-3.5 flex-shrink-0"
                      style={{ color: active ? P.cyan : P.faint, opacity: active ? 1 : 0.7 }} />
                    <span className="text-[0.7rem] font-medium hidden md:block"
                      style={{ color: active ? P.text : P.muted }}>{label}</span>
                  </button>
                );
              })}
